...
class Predictor:
    ...
    @staticmethod
    def _convert_unicode_features_to_tensor(matrix):
    ...
    def predict_coord_center_of_mass(self, model: UnicodeCNN, texts: List[str]):
        """
        Computes a center of mass of top-n $\\mu$-s of an MvMF layer using vMF weights computed for the text.
        """
        unicode_features = torch.stack([Predictor._convert_unicode_features_to_tensor(self._encoder.encode(t)) for t in texts])
    ...
    def predict_coord_grid_search(self, snapshot: str, text: str, num_lat_samples: int, num_lon_samples: int):
        """
        ...
        # 3. unicode encoder
        unicode_features = Predictor._convert_unicode_features_to_tensor(self._encoder.encode(text))
